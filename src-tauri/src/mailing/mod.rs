...
pub async fn mailing_start(
    ...
    if ids.is_empty() {
        return Err(t("mailing_no_accounts"));
    }
    ...
    let voice_bytes: Option<Arc<Vec<u8>>> =
        if config.message_type == "voice" && !config.voice_path.is_empty() {
...
async fn process_account(
    ...
    let emit = |msg: &str| {
        let _ = app.emit("mailing-log", format!("{} {}", prefix, msg));
    };
    ...
    let repost_info: Option<(i64, i32)> = if config.auto_repost && config.message_type == "text" {
        ...
        match client.invoke(&create_req).await {
            Ok(data) => {
                if let Some(chat_id) = tl::parse_created_chat_id(&data) {
                    ...
                    match client.invoke(&send_req).await {
                        ...
                        Err(e) => {
                            emit(&t_with("mailing_repost_send_error", &[("error", &e)]));
                            None
                        }
                    }
                } else {
            ...
            Err(e) => {
                emit(&t_with("mailing_repost_create_error", &[("error", &e)]));
                None
            }
        }
    } else {
    ...
    match config.mode.as_str() {
        "dialogs" => {
            ...
            for peer in peers {
                if !token.load(Ordering::Relaxed) || sent >= effective_limit {
                    break;
                }
                ...
                } = peer
                {
                    ...
                    .await
                    {
                        SendResult::Ok => {
                            sent += 1;
                            db_log_result(db_conn, uid, "", "ok", "", id);
                        }
                        SendResult::Fatal(e) => {
                            db_log_result(db_conn, uid, "", "fatal", &e, id);
                            return Err(e);
                        }
                        SendResult::ErrorBan => {
                            ban_count += 1;
                            db_log_result(db_conn, uid, "", "ban", "", id);
                        }
                        SendResult::ErrorSpamblock => {
                            spamblock_count += 1;
                            db_log_result(db_conn, uid, "", "spamblock", "", id);
                        }
                        SendResult::ErrorFlood => {
                            flood_count += 1;
                            db_log_result(db_conn, uid, "", "flood", "", id);
                        }
                        _ => {
                            db_log_result(db_conn, uid, "", "error", "", id);
                        }
                    }
                    if should_autostop_mailing(config, ban_count, spamblock_count, flood_count) {
                        break;
                    }
                ...
                if let Ok(data) = client.invoke(&resolve_req).await {
                    if let Ok((uid, hash)) = tl::parse_resolved_peer(&data) {
                        ...
                        .await
                        {
                            SendResult::Ok => {
                                sent += 1;
                                db_log_result(db_conn, uid, &usernames[i], "ok", "", id);
                            }
                            SendResult::Fatal(e) => {
                                db_log_result(db_conn, uid, &usernames[i], "fatal", &e, id);
                                return Err(e);
                            }
                            SendResult::ErrorBan => {
                                ban_count += 1;
                                db_log_result(db_conn, uid, &usernames[i], "ban", "", id);
                            }
                            SendResult::ErrorSpamblock => {
                                spamblock_count += 1;
                                db_log_result(db_conn, uid, &usernames[i], "spamblock", "", id);
                            }
                            SendResult::ErrorFlood => {
                                flood_count += 1;
                                db_log_result(db_conn, uid, &usernames[i], "flood", "", id);
                            }
                            _ => {
                                db_log_result(db_conn, uid, &usernames[i], "error", "", id);
                            }
                        }
                        if should_autostop_mailing(config, ban_count, spamblock_count, flood_count)
                        {
        ...
        "chats" => {
            ...
            for target in targets {
                ...
                match crate::mtproto::invite::resolve_channel_link(&mut client, target).await {
                    Ok(resolved) => {
                        let peer = tl_gen::serialize_input_peer_channel(
                            resolved.channel_id,
                        ...
                        let ok = match config.message_type.as_str() {
                            ...
                            "voice" => {
                                if let Some(vb) = voice_bytes {
                                    crate::mtproto::tl::send_voice_message(
                                        &mut client,
                                        resolved.channel_id,
                                        resolved.access_hash,
                                        vb,
                            ...
                            "postbot" => {
                                let req = tl_gen::build_messages_sendMessage(
                                    ...
                                    &peer,
                                    None,
                                    &format!("@PostBot {}", config.postbot_hash),
                                    random_id,
                                    None,
                                    None,
                                    None,
                        ...
                        if ok {
                            emit(&t_with("mailing_sent_to_chat", &[("target", target)]));
                            sent += 1;
                        } else {
                            emit(&t_with("mailing_send_error_chat", &[("target", target)]));
                        }
                        interruptible_sleep(rate_limit_ms(), token).await;
                    }
        ...
        "comments" => {
            ...
            for target in targets {
                ...
                match crate::mtproto::invite::resolve_channel_link(&mut client, target).await {
                    Ok(resolved) => {
                        // get last post
                        let hist_req = tl::build_get_history_channel(
                            resolved.channel_id,
                            resolved.access_hash,
                            1,
                        );
                        let hist_data = match client.invoke(&hist_req).await {
                            Ok(d) => d,
                            Err(e) => {
                                emit(&format!("getHistory {}: {}", target, e));
                                continue;
                            }
                        };
                        let msgs = tl::parse_messages_structured(&hist_data).unwrap_or_default();
                        let msg_id = match msgs.first() {
                            Some(m) => m.id,
                            None => {
                                emit(&t_with("mailing_no_posts_in", &[("target", target)]));
                                continue;
                            }
                        };
                        let text = prepare_text(&config.message_text, &config.text_modify);
                        match crate::first_comment::send_comment_pub(
                            &mut client,
                            resolved.channel_id,
                            resolved.access_hash,
                            msg_id,
                        ...
                        .await
                        {
                            Ok(_) => {
                                emit(&t_with(
                                    "mailing_comment_sent",
                                    &[("target", target), ("msg_id", &msg_id.to_string())],
                                ));
                                sent += 1;
                            }
                            Err(e) => emit(&t_with(
                                "mailing_comment_error",
                        ...
                        Ok(data) => {
                            if let Some((uid, hash)) = tl::parse_imported_contact(&data) {
                                ...
                                .await
                                {
                                    SendResult::Ok => {
                                        sent += 1;
                                        db_log_result(db_conn, uid, phone, "ok", "", id);
                                    }
                                    SendResult::Fatal(e) => {
                                        db_log_result(db_conn, uid, phone, "fatal", &e, id);
                                        return Err(e);
                                    }
                                    SendResult::ErrorBan => {
                                        ban_count += 1;
                                        db_log_result(db_conn, uid, phone, "ban", "", id);
                                    }
                                    SendResult::ErrorSpamblock => {
                                        spamblock_count += 1;
                                        db_log_result(db_conn, uid, phone, "spamblock", "", id);
                                    }
                                    SendResult::ErrorFlood => {
                                        flood_count += 1;
                                        db_log_result(db_conn, uid, phone, "flood", "", id);
                                    }
                                    _ => {
                                        db_log_result(db_conn, uid, phone, "error", "", id);
                                    }
                                }
                                if should_autostop_mailing(
                                    config,
                                    ban_count,
                                    spamblock_count,
                                    flood_count,
                    ...
                    match client.invoke(&resolve_req).await {
                        Ok(data) => {
                            if tl::parse_resolved_peer(&data).is_ok() {
                                    ...
                                    if let Ok(data2) = client.invoke(&resolve_req2).await {
                                        if let Ok((uid, hash)) = tl::parse_resolved_peer(&data2) {
                                            ...
                                            match client.invoke(&req).await {
                                                Ok(_) => {
                                                    emit(&t_with(
                                                        "mailing_story_forwarded",
                                                        &[("username", usernames[i].as_str())],
                                                    ));
                                                    sent += 1;
                                                }
                                                Err(e) => emit(&t_with(
                                                    "mailing_story_forward_error",
                                                    &[
                                                        ("username", usernames[i].as_str()),
                                                        ("error", &e),
                                                    ],
                                ...
                                emit(&t_with(
                                    "mailing_story_owner_resolve_error",
                                    &[("username", story_user.as_str())],
                                ));
                            }
                        }
...
async fn send_to_user(
    ...
    if text.contains('%') {
        let (uname, fname, lname) = if !recipient_username.is_empty()
            ...
            && !text.contains("%LAST_NAME%")
        {
            ...
            match client.invoke(&req).await {
                Ok(data) => tl::parse_user_info(&data).unwrap_or_else(|| {
                    (recipient_username.to_string(), String::new(), String::new())
                }),
    ...
    let result = if let Some((chat_id, msg_id)) = repost_info {
        ...
        match config.message_type.as_str() {
                    ...
                    Ok(resolved) => {
                        match tl::parse_resolved_peer(&resolved) {
                            Ok((bot_id, bot_hash)) => {
                                ...
                                let user_peer =
                                    tl_gen::serialize_input_peer_user(user_id, access_hash);
                                let get_req = tl_gen::build_messages_getInlineBotResults(
                                    &bot_peer,
                                    &user_peer,
                                    None,
                                    &config.postbot_hash,
                                ...
                                match client.invoke(&get_req).await {
                                    Ok(results_data) => {
                                        match tl_gen::parse_messages_getInlineBotResults(
                                            &results_data,
                                        ) {
                                            Ok(bot_results) => {
                                                if bot_results.results.is_empty() {
                                                    ...
                                                    let mut cursor =
                                                        std::io::Cursor::new(first.as_slice());
                                                    match tl_gen::TlBotInlineResult::deserialize(
                                                        &mut cursor,
                                                    ) {
                                                        Ok(result) => {
                                                        ...
                                                        Err(e) => {
                                                            Err(format!("parse inline result: {e}"))
                                                        }
                                                    }
                                                }
                                            }
            ...
            "voice" => {
                if let Some(vb) = voice_bytes {
                    match crate::mtproto::tl::send_voice_message(client, user_id, access_hash, vb)
                        .await
                    {
                        Ok(_) => Ok(Vec::new()),
                        Err(e) => Err(e),
    ...
    match result {
        Ok(resp) => {
            let _ = app.emit(
                ...
                format!(
                    "{} {}",
                    prefix,
                    t_with("mailing_sent_user", &[("user_id", &user_id.to_string())])
                ),
            );
            ...
            if config.pin_message {
                if let Some(msg_id) = tl::extract_sent_msg_id(&resp) {
                    let pin_req =
                        tl::build_pin_message(user_id, access_hash, msg_id, config.silent);
        ...
        Err(e) => {
            if crate::mtproto::is_fatal_session_error(&e) {
                let _ = app.emit(
                    "mailing-log",
                    format!("{} {} user_id={}: {}", prefix, t("error"), user_id, e),
                );
            ...
            let _ = app.emit(
                ...
                format!(
                    ...
                    t_with(
                        "mailing_error_user",
                        &[("user_id", &user_id.to_string()), ("error", &e)]
                    )
                ),
            );
...
fn prepare_text(base: &str, mode: &str) -> String {
    ...
    match mode {
        "llm_rewrite" => crate::llm::complete(&crate::i18n::t("llm_rephrase_prompt"), &spun)
            .unwrap_or_else(|_| spun.clone())
            .trim()
            .to_string(),
...
async fn send_media_to_user(
    ...
    let entities_opt: Option<&[&[u8]]> = if entity_refs.is_empty() {
        None
    } else {
        Some(&entity_refs)
    };
    ...
    let media = if media_type == "video" {
        let video_attr = tl_gen::serialize_documentAttributeVideo(
            video_note, true, false, 0.0, 0, 0, None, None, None,
        );
