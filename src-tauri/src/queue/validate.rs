...
pub async fn enqueue_validate(
    ...
    queue.enqueue(
        ...
        move || {
            Box::pin(async move {
                dbg_log!(
                    "=== VALIDATE TASK START ({} accounts, {} threads) ===",
                    ids_clone.len(),
                    count
                );
                let storage = crate::accounts::commands::get_storage_pub();
                ...
                dbg_log!(
                    "validate task: {} proxies available",
                    proxy_list.proxies.len()
                );
                ...
                for id in ids_clone.iter() {
                    ...
                    handles.push(tokio::spawn(async move {
                        ...
                        let result = tokio::time::timeout(
                            ...
                            async {
                                let storage = crate::accounts::storage::AccountStorage::new(
                                    &storage_base.parent().unwrap(),
                                );
                                ...
                                let session =
                                    match crate::accounts::session::TelethonSession::from_file(
                                        &session_path,
                                    ) {
                                        Ok(s) => s,
                                        Err(_) => {
                                ...
                                let mut json = if json_path.exists() {
                                    crate::accounts::session::AccountJson::from_file(&json_path)
                                        .unwrap_or_default()
                                } else {
                                ...
                                for attempt in 1..=5 {
                                    if attempt > 1 {
                                        dbg_log!(
                                            "validate {}: attempt {}/5 (prev was unreachable)",
                                            id,
                                            attempt
                                        );
                                    ...
                                    let (vr, cli) = crate::checker::validate::validate_account(
                                        &session, &json, proxy_ref,
                                    )
                                    .await;
                                ...
                                json.status = if vr.valid {
                                    if do_r {
                                        ...
                                        let restriction_status =
                                            if let Some(ref mut client) = reusable_client {
                                                match crate::checker::checks::check_spambot(client)
                                                    .await
                                                {
                                                    Ok(s) => s,
                                        ...
                                        let is_temp = prev_status
                                            .starts_with(&crate::i18n::t("status_geo_spam"));
                                ...
                                if do_2fa_check && vr.valid {
                                    if let Some(ref mut client) = reusable_client {
                                        if let Ok((has_2fa, hint)) =
                                            crate::checker::checks::check_2fa_with_hint(client)
                                                .await
                                        {
                                ...
                                if vr.valid {
                                    if let Some(uid) = vr.user_id {
                                        let ts =
                                            crate::accounts::aging::estimate_registration_ts(uid);
                                        if ts > 0 {
