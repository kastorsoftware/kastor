...
pub async fn masslooking_start(
    ...
    if ids.is_empty() {
        return Err(t("masslooking_no_accounts"));
    }
    ...
    let token = queue
        .register_task(
            task_id.clone(),
            "masslooking".to_string(),
            t_with(
                "masslooking_task_name",
...
async fn process_account(
    ...
    let emit = |msg: &str| {
        let _ = app.emit("masslooking-log", format!("{} {}", prefix, msg));
    };
    ...
    for (user_id, access_hash) in &targets {
        ...
        if processed >= config.max_per_account {
            break;
        }
        ...
        if max_id > 0 {
            ...
            emit(&t_with(
                "masslooking_stories_viewed",
                &[
                    ("count", &story_ids.len().to_string()),
        ...
        if config.react_after_view && !story_ids.is_empty() {
            ...
            let react_req =
                build_send_story_reaction(*user_id, *access_hash, story_id, &config.reaction);
            match client.invoke(&react_req).await {
                Ok(_) => emit(&t_with(
                    "masslooking_reaction_sent",
                    &[
                        ("emoji", &config.reaction),
                        ("user_id", &user_id.to_string()),
                    ],
                )),
                Err(e) => {
        ...
        if config.reply_to_story && !config.reply_text.is_empty() && !story_ids.is_empty() {
            ...
            let reply_req =
                build_send_story_reply(*user_id, *access_hash, story_id, &text, random_id);
            match client.invoke(&reply_req).await {
                Ok(_) => emit(&t_with(
                    "masslooking_reply_sent",
                    &[("user_id", &user_id.to_string())],
                )),
                Err(e) => {
                    if crate::mtproto::is_fatal_session_error(&e) {
    ...
    emit(&t_with(
        "masslooking_processed",
        &[("count", &processed.to_string())],
    ));
...
async fn collect_targets(
    ...
    match config.source_mode.as_str() {
        ...
        "chat" => {
            let resolved =
                crate::mtproto::invite::resolve_channel_link(client, &config.chat_target).await?;
...
fn prepare_reply_text(base: &str, mode: &str) -> String {
    match mode {
        "llm_rewrite" => match crate::llm::complete(&crate::i18n::t("llm_rephrase_prompt"), base) {
