...
pub async fn stories_start(
    ...
    let links_file: Option<Arc<std::sync::Mutex<std::fs::File>>> =
        if !config.output_links_path.is_empty() {
...
#[tauri::command]
pub async fn stories_stop(task_id: String, app: tauri::AppHandle) -> Result<(), String> {
    let queue: tauri::State<'_, TaskQueue> = app.state();
...
async fn process_account(
    ...
    let json = AccountJson::from_file(&json_path).map_err(|e| format!("json: {e}"))?;
    ...
    let max_stories = if config.stories_min > 0 && config.stories_max > 0 {
        ...
        if lo == hi {
            ...
            (lo + rand::random::<u32>() % (hi - lo + 1)) as usize
        }
    } else {
    ...
    for story_idx in 0..stories_to_upload {
        let story_tags = if !account_tags.is_empty() && per_story_tags > 0 {
            ...
            if start >= account_tags.len() {
                Vec::new()
            } else {
                account_tags[start..end].to_vec()
            }
        } else {
            Vec::new()
        };
        ...
        let file_name = if config.media_type == "photo" {
            "story.jpg"
        } else {
            "story.mp4"
        };
                    ...
                    if let Some(secs) = extract_flood_wait(&e) {
                        if config.max_flood_wait > 0 && secs > config.max_flood_wait {
                            return Err(t_with(
                                ...
                                &[
                                    ("secs", &secs.to_string()),
                                    ("limit", &config.max_flood_wait.to_string()),
                                ],
                            ));
                        }
    ...
    let tagged = if account_tags.is_empty() {
        String::new()
    } else {
        t_with(
            "stories_tags_suffix",
            &[("count", &account_tags.len().to_string())],
        )
    };
    Ok(t_with(
        "stories_uploaded",
        &[("count", &uploaded.to_string()), ("tags", &tagged)],
    ))
}
