...
#[tauri::command]
pub async fn checker_sort_results(
    ...
    let categories: Vec<(
        &str,
        Box<dyn Fn(&CheckerAccountForSort) -> bool + Send + Sync>,
    )> = vec![
        (
            "With_NFT_Tags",
            Box::new(|a: &CheckerAccountForSort| !a.nft_tags.is_empty()),
        ),
        ("With_888", Box::new(|a: &CheckerAccountForSort| a.phone888)),
        (
            "With_Channels",
            Box::new(|a: &CheckerAccountForSort| !a.channels.is_empty()),
        ),
        (
            "With_NFT_Gifts",
            Box::new(|a: &CheckerAccountForSort| !a.nft_gifts.is_empty()),
        ),
        ("Premium", Box::new(|a: &CheckerAccountForSort| a.premium)),
        (
            "With_Seed",
            Box::new(|a: &CheckerAccountForSort| a.seed_found),
        ),
        (
            "With_Pass_Files",
            Box::new(|a: &CheckerAccountForSort| !a.pass_files.is_empty()),
        ),
        (
            "With_Channel_Balances",
            Box::new(|a: &CheckerAccountForSort| !a.channel_balances.is_empty()),
        ),
        (
            "Temp_Spamblock",
            Box::new(|a: &CheckerAccountForSort| a.spamblock == "temp_geo"),
        ),
        (
            "Perm_Spamblock",
            Box::new(|a: &CheckerAccountForSort| a.spamblock == "perm"),
        ),
        (
            "Frozen",
            Box::new(|a: &CheckerAccountForSort| a.spamblock == "frozen"),
        ),
        // Valid_No_Restrictions: only clean accounts that don't have any special attributes
        (
            "Valid_No_Restrictions",
            Box::new(|a: &CheckerAccountForSort| {
                a.spamblock == "none"
                    && a.nft_tags.is_empty()
                    && !a.phone888
                    && a.channels.is_empty()
    ...
    let parsed: Vec<CheckerAccountForSort> = accounts
        .iter()
    ...
    for account in &parsed {
        ...
        if source.exists() {
            ...
            if canonical_dest.starts_with(&canonical_source) {
                return Err(
                    "sort destination must not be inside a source account directory".into(),
                );
            }
        }
    }
    ...
    for (folder_name, predicate) in &categories {
        let matching: Vec<&CheckerAccountForSort> =
            parsed.iter().filter(|a| predicate(a)).collect();
            ...
            if is_session {
                ...
                if let Ok((auth_key, dc_id)) = session_result {
                    ...
                    if user_id != 0 {
                        ...
                        let tdata_out =
                            tdatas_dir.join(format!("{}_tdata", stem.trim_start_matches('+')));
