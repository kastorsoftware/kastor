...
fn default_account_threads() -> u32 {
    5
}
...
fn default_converter_threads() -> u32 {
    10
}
fn default_checker_channels_min() -> u32 {
    100
}
fn default_checker_groups_min() -> u32 {
    100
}
fn default_reauth_threads() -> u32 {
    5
}
fn default_account_actions_threads() -> u32 {
    5
}
fn default_llm_api_type() -> String {
    "openai".to_string()
}
...
pub fn patch_settings(patch: serde_json::Value) -> Result<(), String> {
    ...
    patch_u32(
        &patch,
        "checker_channels_min",
        &mut current.checker_channels_min,
    )?;
    patch_u32(
        &patch,
        "checker_groups_min",
        &mut current.checker_groups_min,
    )?;
    patch_u32(&patch, "reauth_threads", &mut current.reauth_threads)?;
    patch_u32(
        &patch,
        "account_actions_threads",
        &mut current.account_actions_threads,
    )?;
    if let Some(v) = patch.get("llm_api_url").and_then(|v| v.as_str()) {
        current.llm_api_url = v.to_string();
    }
    if let Some(v) = patch.get("llm_token").and_then(|v| v.as_str()) {
        current.llm_token = v.to_string();
    }
    if let Some(v) = patch.get("llm_model").and_then(|v| v.as_str()) {
        current.llm_model = v.to_string();
    }
    if let Some(v) = patch.get("llm_api_type").and_then(|v| v.as_str()) {
        current.llm_api_type = v.to_string();
    }
    if let Some(v) = patch.get("validate_check_2fa").and_then(|v| v.as_bool()) {
        current.validate_check_2fa = v;
    }
    if let Some(v) = patch.get("validate_check_aging").and_then(|v| v.as_bool()) {
        current.validate_check_aging = v;
    }
    current.save()
}
