...
async fn run(
    ...
    if group_link.is_empty() {
        return Err(t("forwarder_no_group"));
    }
    ...
    emit(
        ...
        t_with(
            "forwarder_start",
            &[("total", &total.to_string()), ("group", group_link)],
        ),
    );
    ...
    for (idx, account_id) in account_ids.into_iter().enumerate() {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let emit = |msg: String| {
                let _ = app_clone.emit("forwarder-log", format!("{} {}", prefix, msg));
            };
            ...
            let resolved = match crate::mtproto::invite::resolve_channel_link(
                &mut client,
                &cfg_clone.group_link,
            )
            .await
            {
                Ok(r) => r,
                Err(e) => {
                    emit(t_with("forwarder_resolve_error", &[("error", &e)]));
                    return;
                }
            };
            ...
            let (mut pts, mut date, mut qts) = match client.invoke(&state_req).await {
                Ok(data) => match tl_gen::parse_updates_getState(&data) {
                    ...
                    Err(e) => {
                        emit(t_with("forwarder_getstate_parse_error", &[("error", &e)]));
                        return;
                    }
                },
                Err(e) => {
                    emit(t_with(
                        "forwarder_getstate_error",
                        &[("error", &e.to_string())],
                    ));
                    return;
                }
            ...
            if cfg_clone.resend_old {
                                    ...
                                    if let Ok(msgs) = tl::parse_messages_structured(&hist_data) {
                                        if let Some(msg) = msgs.first() {
                                            ...
                                            let delay = random_delay_secs(
                                                cfg_clone.resend_old_wait_min,
                                                cfg_clone.resend_old_wait_max,
                                            );
                                            ...
                                            if !joined {
                                                let join_req = tl_gen::build_channels_joinChannel(
                                                    &input_channel,
                                                );
                                                if client.invoke(&join_req).await.is_ok() {
                                                    joined = true;
                                                }
                                            }
                                            ...
                                            let to_peer = tl_gen::serialize_input_peer_channel(
                                                group_id, group_hash,
                                            );
                                            ...
                                            if let Ok(resp) = client.invoke(&send_req).await {
                                                if let Some(fwd_id) =
                                                    extract_forwarded_msg_id(&resp)
                                                {
                                                    forward_map.insert(
                                                        fwd_id,
                                                        ForwardedInfo {
                                                            sender_id: *uid,
                                                            sender_access_hash: *uhash,
                                                        },
                                                    );
                ...
                emit(t_with(
                    "forwarder_old_forwarded",
                    &[(
                        "count",
                        &forwarded_count.load(Ordering::Relaxed).to_string(),
                    )],
                ));
            }
            ...
            loop {
                ...
                let diff_req =
                    tl_gen::build_updates_getDifference(pts, None, None, date, qts, None);
                ...
                let (new_messages, other_updates, users, new_state) = match diff {
                    tl_gen::TlUpdatesDifference::Empty { date: d, .. } => {
                        date = d;
                        continue;
                    }
                    tl_gen::TlUpdatesDifference::Difference {
                        ...
                        state,
                        ..
                    } => (new_messages, other_updates, users, Some(state)),
                    tl_gen::TlUpdatesDifference::Slice {
                        ...
                        intermediate_state,
                        ..
                    } => (new_messages, other_updates, users, Some(intermediate_state)),
                    tl_gen::TlUpdatesDifference::TooLong { pts: p } => {
                        pts = p;
                        continue;
                    }
                };
                ...
                if let Some(state_raw) = new_state {
                    if let Ok(s) =
                        tl_gen::TlUpdatesState::deserialize(&mut Cursor::new(state_raw.as_slice()))
                    {
                        pts = s.pts;
                        date = s.date;
                        qts = s.qts;
                ...
                for msg_raw in &all_new_messages {
                    ...
                    let msg = match tl_gen::TlMessage::deserialize(&mut Cursor::new(
                        msg_raw.as_slice(),
                    )) {
                        Ok(m) => m,
                        Err(_) => continue,
                    ...
                    match msg {
                        tl_gen::TlMessage::Message {
                            ...
                            let peer = match tl_gen::TlPeer::deserialize(&mut Cursor::new(
                                peer_id.as_slice(),
                            )) {
                                Ok(p) => p,
                                Err(_) => continue,
                            ...
                            match peer {
                                tl_gen::TlPeer::User {
                                    ...
                                    let sender_from = if let Some(ref fid) = from_id {
                                        match tl_gen::TlPeer::deserialize(&mut Cursor::new(
                                            fid.as_slice(),
                                        )) {
                                    ...
                                    let sender_hash =
                                        user_map.get(&sender_from).copied().unwrap_or(0);
                                    if sender_hash == 0 {
                                        continue;
                                    }
                                    ...
                                    if !joined {
                                        let join_req =
                                            tl_gen::build_channels_joinChannel(&input_channel);
                                    ...
                                    let mess_delay = random_delay_secs(
                                        cfg_clone.mess_wait_min,
                                        cfg_clone.mess_wait_max,
                                    );
                                    if mess_delay > 0 {
                                        interruptible_sleep_ms(
                                            mess_delay as u64 * 1000,
                                            &token_clone,
                                        )
                                        .await;
                                        if !token_clone.load(Ordering::Relaxed) {
                                    ...
                                    let sender_name = user_name_map
                                        .get(&sender_from)
                                        .cloned()
                                        .unwrap_or_default();
                                    let sender_uname = user_username_map
                                        .get(&sender_from)
                                        .cloned()
                                        .unwrap_or_default();
                                    ...
                                    let to_peer =
                                        tl_gen::serialize_input_peer_channel(group_id, group_hash);
                                    let rid: i64 = rand::random();
                                    let send_req = tl_gen::build_messages_sendMessage(
                                        false,
                                        false,
                                    ...
                                    match client.invoke(&send_req).await {
                                        Ok(resp) => {
                                            if let Some(fwd_id) = extract_forwarded_msg_id(&resp) {
                                                forward_map.insert(
                                                    fwd_id,
                                                    ForwardedInfo {
                                                        sender_id: sender_from,
                                                        sender_access_hash: sender_hash,
                                                    },
                                                );
                                                forwarded_count.fetch_add(1, Ordering::Relaxed);
                                                emit(t_with(
                                        ...
                                        Err(e) => {
                                            emit(t_with(
                                                "forwarder_forward_error",
                                                &[("error", &e.to_string())],
                                            ));
                                        }
                                    }
                                }
                                tl_gen::TlPeer::Channel {
                                    channel_id: chan_id,
                                } => {
                                    ...
                                    let send_delay = random_delay_secs(
                                        cfg_clone.send_wait_min,
                                        cfg_clone.send_wait_max,
                                    );
                                    if send_delay > 0 {
                                        interruptible_sleep_ms(
                                            send_delay as u64 * 1000,
                                            &token_clone,
                                        )
                                        .await;
                                        if !token_clone.load(Ordering::Relaxed) {
                                    ...
                                    // typing simulation
                                    let user_peer = tl_gen::serialize_input_peer_user(
                                        info.sender_id,
                                        info.sender_access_hash,
                                    );
                                    let typing_action = serialize_typing_action();
                                    let typing_req = tl_gen::build_messages_setTyping(
                                        &user_peer,
                                        None,
                                        &typing_action,
                                    );
                                    ...
                                    let typing_secs = random_delay_secs(
                                        cfg_clone.typing_min,
                                        cfg_clone.typing_max,
                                    );
                                    interruptible_sleep_ms(typing_secs as u64 * 1000, &token_clone)
                                        .await;
                                    if !token_clone.load(Ordering::Relaxed) {
                                        break;
                                    }
                                    ...
                                    match copy_result {
                                        Ok(_) => {
                                            ...
                                            emit(t_with(
                                                "forwarder_reply_copied",
                                                &[("user_id", &info.sender_id.to_string())],
                                            ));
                                        }
                                        Err(e) => {
                                            emit(t_with("forwarder_copy_error", &[("error", &e)]));
                                        }
                                    }

                                    // send reaction if enabled
                                    if cfg_clone.send_reaction {
                                        let group_peer = tl_gen::serialize_input_peer_channel(
                                            group_id, group_hash,
                                        );
                                        let reaction_bytes = tl_gen::serialize_reactionEmoji("🕊");
                                        let reaction_refs: &[&[u8]] = &[&reaction_bytes];
                                        let react_req = tl_gen::build_messages_sendReaction(
                                            false,
                                            true,
                                            &group_peer,
                                            id,
...
async fn copy_message_to_user(
    ...
    if let Some(media_raw) = media {
        ...
        if let Some(input_media) = parse_media_to_input(media_raw) {
            let ent_refs: Vec<&[u8]> = entities
                .as_ref()
                .map(|v| v.iter().map(|e| e.as_slice()).collect())
                .unwrap_or_default();
            let ent_opt: Option<&[&[u8]]> = if ent_refs.is_empty() {
                None
            } else {
                Some(&ent_refs)
            };
...
async fn send_text_message(
    ...
    let req = tl_gen::build_messages_sendMessage(
        false, false, false, false, false, false, false, false, peer, None, text, random_id, None,
        ent_opt, None, None, None, None, None, None, None, None,
    );
    client
        .invoke(&req)
        .await
...
fn parse_media_to_input(media_raw: &[u8]) -> Option<Vec<u8>> {
    ...
    match media {
        tl_gen::TlMessageMedia::Photo {
            photo: Some(photo_raw),
            ..
        } => {
            let photo =
                tl_gen::TlPhoto::deserialize(&mut Cursor::new(photo_raw.as_slice())).ok()?;
            match photo {
                tl_gen::TlPhoto::Photo {
                    id,
                    access_hash,
                    file_reference,
                    ..
                    ...
                    let input_photo =
                        tl_gen::serialize_inputPhoto(id, access_hash, &file_reference);
                    Some(tl_gen::serialize_inputMediaPhoto(
                        false,
                        false,
                        &input_photo,
                        None,
        ...
        tl_gen::TlMessageMedia::Document {
            document: Some(doc_raw),
            ..
        } => {
            ...
            match doc {
                tl_gen::TlDocument::Document {
                    ...
                    let input_doc =
                        tl_gen::serialize_inputDocument(id, access_hash, &file_reference);
                    Some(tl_gen::serialize_inputMediaDocument(
                        false, &input_doc, None, None, None, None,
                    ))
                }
                _ => None,
            }
