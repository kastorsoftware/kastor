...
async fn run(
    ...
    if skipped > 0 {
        emit(
            app,
            t_with(
                "user_lookup_duplicates_skipped",
        ...
        handles.push(tokio::spawn(async move {
            ...
            if let Err(e) = result {
                ...
                let _ = app_clone.emit(
                    "user-lookup-log",
                    t_with(
                        "user_lookup_account_error",
                        &[
                            ("idx", &(acc_idx + 1).to_string()),
                            ("error", &e.to_string()),
                        ],
                    ),
                );
...
async fn process_account_batch(
    ...
    for (idx, target) in batch {
        ...
        let personal_channel = if cfg.save_personal_channel {
            fetch_personal_channel(&mut client, user_id, access_hash)
                .await
                .unwrap_or_default()
        } else {
            String::new()
...
async fn resolve_by_username(
    ...
    let data = client.invoke(&req).await.map_err(|e| {
        ...
        t_with(
            "user_lookup_resolve_username_error",
            &[("error", &e.to_string())],
        )
    })?;
    ...
    let info = parse_user_from_resolve_response(&data)
        .map_err(|_| t_with("user_lookup_channel_skip", &[("username", username)]))?;
    Ok((id, hash, info))
}
...
async fn resolve_by_phone(
    ...
    let data = client.invoke(&req).await.map_err(|e| {
        ...
        t_with(
            "user_lookup_resolve_phone_error",
            &[("error", &e.to_string())],
        )
    })?;
    let (id, hash) = tl::parse_resolved_peer(&data).map_err(|e| {
    ...
    let info = parse_user_from_resolve_response(&data).unwrap_or_else(|_| minimal_user_info(id));
    Ok((id, hash, info))
}
...
async fn fetch_nft_gifts(
    ...
    let req = tl_gen::build_payments_getSavedStarGifts(
        false, false, false, false, false, false, false, false, false, &peer, None, "", 100,
    );
    let data = client
        .invoke(&req)
        .await
...
async fn fetch_personal_channel(
    client: &mut MtpClient,
    user_id: i64,
    access_hash: i64,
) -> Result<String, String> {
...
fn format_found_line(
    ...
    if cfg.save_name {
        parts.push(if info.first_name.is_empty() {
            String::new()
        } else {
    ...
    if cfg.save_surname {
        parts.push(if info.last_name.is_empty() {
            String::new()
        } else {
            info.last_name.clone()
        });
    }
    if cfg.save_username {
        parts.push(if info.username.is_empty() {
            String::new()
        } else {
            info.username.clone()
        });
    ...
    if cfg.save_phone {
        parts.push(if info.phone.is_empty() {
            String::new()
        } else {
            info.phone.clone()
        });
    }
    if cfg.save_nft_gifts {
        parts.push(nft_links.join(";"));
    }
    if cfg.save_personal_channel {
        parts.push(personal_channel.to_string());
    }
    parts.join(":")
}
